 ...
 */
#define W25Q128JV_DUMMY_BYTE                    0xA5
...
{
    SERCOM6_REGS->SPIM.SERCOM_DATA = data;
    while (!(SERCOM6_REGS->SPIM.SERCOM_INTFLAG & SERCOM_SPIM_INTFLAG_TXC_Msk));
    return SERCOM6_REGS->SPIM.SERCOM_DATA & 0xFF;
}
...
uint8_t w25q128jv_init(void)
{
    ...
    printf("Device ID      : 0x%02x%02x\n", (uint8_t)((id & 0xFF00) >> 8), (uint8_t)(id & 0xFF));
...
{
    __w25q128jv_CS_ENABLE();
    w25q128jv_spi(W25Q128JV_READ_DATA); // TODO: 0x0B fast read.
    w25q128jv_spi((addr & 0xFF0000) >> 16);
    w25q128jv_spi((addr & 0xFF00) >> 8);
    w25q128jv_spi(addr & 0xFF);
    w25q128jv_spi(0);
